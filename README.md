...
Bu proje, tez ve makale yazımında AI destekli fakat insan denetimli akademik üretim için hazırlanmış modüler bir `skills/tr` ve `skills/en` paketidir. Amaç tek bir uzun prompt yazmak değil; tez yazımı, makale yazımı, üslup düzeltme, iddia–kanıt denetimi, kaynakça denetimi ve LaTeX format kontrolünü ayrı ama birlikte çalışabilen skill'ler olarak düzenlemektir.
...
| Prior work ignores domain-specific constraints. | Literature review incomplete. | needs evidence | Add citations or weaken. |
...
```bash
GITHUB_TOKEN=... python3 tools/check_upstream_updates.py \
...
5. python3 tools/check_skill_suite.py çalıştır.
...
- Beklenen `skills/tr` ve `skills/en` dizinleri ve 10 skill dizini var mı?
...
- `skills/_shared` altındaki ortak referans ve şablonlar mevcut mu?
...
Akademik yazım kuralları ve tez biçim kuralları bağlama duyarlıdır. Upstream değişiklikler faydalı olabilir; fakat doğrudan merge edilirse yerel tez dili, OMÜ formatı veya bilgisayar bilimleri / ampirik yazılım mühendisliği duyarlı denetimler bozulabilir. Bu nedenle otomatik rapor + insan kontrollü port yaklaşımı tercih edilmiştir.
